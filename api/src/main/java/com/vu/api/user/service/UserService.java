...
public class UserService {
    ...
    public UserResponse create(UserCreateRequest req) {
        ...
        Role role = roleRepo.findByNameIgnoreCase("USER").orElseThrow(() -> new ApiException(ErrorCode.ROLE_NOT_FOUND));
        ...
        User refreshed =
                userRepo.findByIdWithRoles(u.getId()).orElseThrow(() -> new ApiException(ErrorCode.USER_NOT_FOUND));
    ...
    @PostAuthorize(
            " returnObject.email == authentication.name or hasRole('ADMIN') ") // USER chỉ được xem thông tin của chính
    // mình, ADMIN được xem tất cả
    @Transactional(readOnly = true)
    public UserResponse getById(Long id) {
        User u = userRepo.findByIdWithRoles(id).orElseThrow(() -> new ApiException(ErrorCode.USER_NOT_FOUND));
        return userMapper.toResponse(u);
    }
    ...
    public UserResponse assignRole(Long userId, String roleName) {
        userRepo.findByIdWithRoles(userId).orElseThrow(() -> new ApiException(ErrorCode.USER_NOT_FOUND));

        Role role =
                roleRepo.findByNameIgnoreCase(roleName).orElseThrow(() -> new ApiException(ErrorCode.ROLE_NOT_FOUND));
    ...
    public UserResponse update(Long id, com.vu.api.user.DTO.request.UserUpdateRequest req) {
        ...
        User refreshed = userRepo.findByIdWithRoles(id).orElseThrow(() -> new ApiException(ErrorCode.USER_NOT_FOUND));
        return userMapper.toResponse(refreshed);
    }
}
