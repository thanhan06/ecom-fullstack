...
public class AuthenticationController {
    ...
    ResponseEntity<ApiResponse<AuthenticationResponse>> login(
            @RequestBody AuthenticationRequest req, HttpServletRequest httpReq) throws JOSEException {
        return ApiResponses.ok(httpReq, service.login(req));
    }

    @PostMapping("/introspect")
    ResponseEntity<ApiResponse<IntrospectResponse>> login(
            @RequestBody IntrospectRequest req, HttpServletRequest httpReq) throws JOSEException, ParseException {
        return ApiResponses.ok(httpReq, service.introspect(req));
    }
    // spotless:off
    @PostMapping("/logout")
    ResponseEntity<ApiResponse<Void>> logout(@RequestBody LogoutRequest req, HttpServletRequest httpReq) throws ParseException, JOSEException {
        service.logout(req);
...
// spotless:on
    ...
    ResponseEntity<ApiResponse<AuthenticationResponse>> refresh(
            @RequestBody RefreshRequest req, HttpServletRequest httpReq) throws ParseException, JOSEException {
