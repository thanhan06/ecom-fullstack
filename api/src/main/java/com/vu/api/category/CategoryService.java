...
public class CategoryService {
    ...
    public CategoryResponse update(Long id, CategoryUpdateRequest req) {
        Category existing = repo.findById(id).orElseThrow(() -> new ApiException(ErrorCode.CATEGORY_NOT_FOUND));

        if (!existing.getName().equalsIgnoreCase(req.name()) && repo.existsByNameIgnoreCase(req.name())) {
            throw new ApiException(ErrorCode.CATEGORY_NAME_EXISTS);
        }
